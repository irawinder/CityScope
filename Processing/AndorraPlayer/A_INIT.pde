...
void CDRNetwork() {
  ...
  for (int i=0; i<numSwarm; i++) {
   if (network.getInt(i, "CON_O") == 0 && network.getInt(i, "CON_D") == 0) {  
            ...
            origin_coord[i] = mercatorMap.getScreenLocation(new PVector(marc_rest.getFloat(j, "LAT"), marc_rest.getFloat(j, "LNG")));
            destination_coord[i] = mercatorMap.getScreenLocation(new PVector(marc_rest.getFloat(j, "LAT"), marc_rest.getFloat(j, "LNG")));
            ...
            destination_coord[j] = new PVector(destination_coord[j].x + marginWidthPix, destination_coord[j].y + marginWidthPix);
             ...
             minDistance = pow((origin_coord[j].x - origin[1].x), 2)  +  pow((origin_coord[j].y - origin[1].y), 2);
             minIndex = 1;
             
             for(int i=0; i<numSwarm; i++){
               if (network.getInt(i, "CON_O") == 0 && network.getInt(i, "CON_D") == 0) {  
              float dist = pow((origin_coord[j].x - origin[i].x), 2)  +  pow((origin_coord[j].y - origin[i].y), 2);
